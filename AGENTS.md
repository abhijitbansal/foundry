...
- **Consequence for new repos:** once a repo is in the archive it is permanent, so an allowlisted repo with no `YARD` slot in `src/lib/works-layout.ts` fails the build until someone adds one. That's the intended loud failure, just no longer self-healing after a purge. `cubby-site`, `paperix-site`, `floorprint-site`, `mr_lender` and `second-wind` are allowlisted with no slot today — one session in any of them arms this. Adding the slot is the fix; the escape hatch is dropping the repo from `PROJECTS_ALLOWLIST`, which now deletes its archive row on the next run rather than merely hiding it.
- **The allowlist is enforced twice on purpose** — when merging into the archive and again when writing `data/stats.json`. The public file's guarantee must not depend on the archive file being clean, and a repo removed from `PROJECTS.md` has to actually disappear rather than being skipped over and republished forever.
- **A vacant lot is a data state, not a layout property.** `arch: 'lot'` in `YARD` is a default, not a decision: `buildingEls` draws the lot when `storeys <= 0` and a real silhouette otherwise, so the plan can't announce "62,418 lines added" over bare dirt (folix did, at rank 03, for two months).
- **Nameplates need a browser, not arithmetic.** The yard paints back-to-front; a badge parked where a nearer building's roof lands is painted over, and a single `elementFromPoint` at its centre still passes over a 0.5px hairline through the digits. Sample the whole badge disc in a real browser after moving one. `yardFootprints()` (`src/lib/works.ts`) plus its unit test covers building and annex footprints only — not chimney `stacks` (which sit outside their parent's box), not `np` positions, not the crane/rail/flatcar furniture, and not screen-space occlusion.
